...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("client_gestures").insert({
    control_id: client.control_id,
    client_id: client.id,
    calendar_workspace_id: workspace?.id ?? null,
    title: String(body.title).trim(),
    description: body.description ? String(body.description).trim() : null,
    gesture_type: body.gestureType || "task",
    status: "planned",
    starts_at: startsAt.toISOString(),
    ends_at: endsAt.toISOString(),
    timezone: workspace?.timezone || "America/Santiago",
    ...
    metadata: { client_name: client.name, created_from: "dashboard" },
  }).select("*").single();
  ...
  await supabase.from("event_bus").insert({
    ...
    event_type: "gesture.created",
    entity_type: "gesture",
    global_id: `client:${client.id}`,
    dedupe_key: `gesture.created:${data.id}`,
    payload: { gesture_id: data.id, client_id: client.id, title: data.title, starts_at: data.starts_at, sync_status: data.sync_status },
