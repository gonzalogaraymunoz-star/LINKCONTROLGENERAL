...
export async function POST(request: NextRequest) {
  ...
  if (action === "create_client") {
    ...
    const { data, error } = await supabase.from("clients").insert({ control_id: ROOT_CONTROL_ID, name, slug, short_code: shortCode, accent, status: "active", metadata: { source: "link-control-app" } }).select().single();
  ...
  if (action === "update_cycle") {
    ...
    const { error } = await supabase.from("client_cycles").update(patch).eq("id", cycleId);
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  }

  if (action === "update_need") {
    const { error } = await supabase.from("needs").update({ title: String(body.title || ""), description: body.description || null, updated_at: new Date().toISOString() }).eq("id", String(body.needId || ""));
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  }

  if (action === "update_product") {
    const { error } = await supabase.from("products").update({ name: String(body.name || ""), description: body.description || null, updated_at: new Date().toISOString() }).eq("id", String(body.productId || ""));
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  }
  ...
  if (action === "archive_client") {
    const { error } = await supabase.from("clients").update({ status: "archived", archived_at: new Date().toISOString() }).eq("id", String(body.clientId || ""));
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  }
